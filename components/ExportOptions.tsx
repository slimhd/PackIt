...
const ExportOptions = ({ className = '' }: ExportOptionsProps) => {
  ...
  const handleEmailSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const packingListText = `Packing List for ${destination}

Trip Details:
- Destination: ${destination}
- Start Date: ${formatDate(startDate)}
- End Date: ${formatDate(endDate)}
- Packing Style: ${isLightPack ? 'Light Pack' : 'Regular Pack'}${weatherInfo}

Packing List:
${packingList.map((item, index) => `${index + 1}. ${item.name}${item.category ? ` (${item.category})` : ''}`).join('\n')}

Generated by PackWise - Your AI-powered packing assistant
Visit: packwise.app`;
